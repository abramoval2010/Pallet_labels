...
def prepare_result_data(filepath):
    """
    Общая логика: парсит PDF и готовит данные для result.html или для генерации этикеток.
    Возвращает кортеж (data, total_pallets, error_message).
    Если error_message не None — рендерить result.html нельзя.
    """
    data = extract_data_from_pdf(filepath)

    if not data or (not data.get('дата_приемки') and not data.get('название_поставщика') and not data.get('товары')):
        return None, 0, 'Не удалось извлечь данные из PDF-файла. Проверьте формат файла.'
    ...
    return data, total_pallets, None
